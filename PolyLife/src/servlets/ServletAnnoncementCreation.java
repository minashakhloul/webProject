...
public class ServletAnnoncementCreation extends HttpServlet {
	...
	private void eventCreation(HttpServletRequest request) {
		...
		try {
			...
			String req = "INSERT INTO annonce (idEtudiant,titre,description,prix) "
					+ "VALUES ('" + idUser + "','" + title + "','" + description + "','" + price + "')";
			...
			if(mRequest.getFile("imgAnnoncement") != null) {
				...
				if(idAnnoncement != -1) {
					...
					req = "INSERT INTO annoncephoto (idAnnonce, urlPhoto) "
							+ "VALUES ('" + idAnnoncement + "',\"" + loadedFilePath + "\")";
					System.out.println(req);
