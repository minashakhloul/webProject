...
public class ServletEventCreation extends HttpServlet {
	...
	private void eventCreation(HttpServletRequest request) {
    	...
    	try {
			...
			String req = "INSERT INTO evenement (idEtudiant,titre,date,heure,description) "
					+ "VALUES ('" + idUser + "','" + title + "'," 
					+ "STR_TO_DATE('"+ day + "." + month + "." + year + "','%d.%m.%Y')" + ",'" 
					+ hour + ":" + minutes + "','" + description + "')";
			con.insertData(req);
			...
			if(mRequest.getFile("imgEvent") != null) {
				...
				if(idEvent != -1) {
					...
					req = "INSERT INTO evenementphoto (idEvenement, urlPhoto) VALUES ('" 
							+ idEvent + "',\"" + loadedFilePath + "\")";
