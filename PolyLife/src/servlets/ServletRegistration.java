...
public class ServletRegistration extends HttpServlet {
	...
	public static final String signInPage = "/login.jsp";
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!userCreation(request)) {
			...
			this.getServletContext().getRequestDispatcher(signInPage).forward(request, response);
			// try {
			// con.destroy();
			// } catch (SQLException e) {
	...
	private boolean userCreation(HttpServletRequest request) {
		...
		case STUDENT:
			return studentCreation(request);
		case EX_STUDENT:
			return exStudentCreation(request);
		case PROFESSOR:
			return professorCreation(request);
		}
		return false;
	}
	...
	private boolean studentCreation(HttpServletRequest request) {
		...
		if (!studentExist(emailAdress)) {
			String req = "INSERT INTO etudiant (nom,prenom,email,password,dateDeNaissance,nationalite,sex,telephone," + "idStatusEtudiant,"
					+ "idSpecialite,promotion,annee) VALUES ('"
					+ name
					+ "','"
					+ firstName
					+ "','"
					...
					+ idStatusStudent
					+ "','"
					+ idSpeciality + "','" + Integer.parseInt(promotion) + "','" + Integer.parseInt(currentYear) + "')";
			con.insertData(req);
