...
<div id="headcontentzone">
	<div class="page-container">
		<table class="table table-striped table-hover ">
			...
			<c:forEach items="${EventsUser}" var="event">
				<tr class="info">
					<td><c:out value="${event.value.getTitle()}">
						</c:out></td>
					<td><c:out value="${event.value.getDescription()}">
						</c:out></td>
					<td>
						<button
							onclick="window.location='/PolyLife/protected/modifierEvenement.jsp'">Modifier
							évènement</button>
						<form name="message" action="/PolyLife/ListeEvenements"
							method="get">
							<input name="delete" type="submit" id="delete" value="delete" />
							<input type="hidden" name="idEvenement"
								value="${event.value.getIdEvent()}" />
						</form>
					</td>
